...
def get_worker_by_chat(chat_id):
    try:
        r = supabase.table("workers").select("*").eq("chat_id", chat_id).execute()
...
async def login_pass(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if role == "admin":
        ...
        r = supabase.table("workers").select("*").eq("login", li).eq("role", role).execute()
        w = r.data[0] if r.data else None
        if not w or w.get("pass") != pwd:
            await update.message.reply_text("❌ Login yoki parol noto'g'ri!\n/start ni bosing.")
            return LOGIN_ROLE
        supabase.table("workers").update({"chat_id": chat_id}).eq("id", w["id"]).execute()
...
async def do_home(update, context):
    ...
    sales = supabase.table("sales").select("*").eq("sale_date", today()).execute().data or []
    clients = supabase.table("clients").select("*").execute().data or []
    workers = supabase.table("workers").select("*").execute().data or []
    ...
    txt += f"👔 Ombordagi koylak: *{koylak} dona*\n"
    txt += f"🧵 Material: *{mat} metr*\n"
    txt += f"⚠️ Mijozlar qarzi: *{fmt(debt)}*\n\n"
    txt += f"📦 *Ombor qiymati:*\n"
    txt += f"  👔 {fmt(kv)}\n  🧵 {fmt(mv)}\n  🔩 {fmt(dv)}\n"
    txt += f"  💎 Jami: *{fmt(kv+mv+dv)}*\n\n"
    txt += f"👷 Ishchilar: {len(workers)} ta\n"
    if sales:
        txt += f"\n🛒 *Bugun {len(sales)} ta sotuv:*\n"
        for s in sales[-5:]: txt += f"  • {s['product_name']} ×{s['qty']} = {fmt(s['total'])}\n"
...
async def add_prod_price(update, context):
    try: context.user_data["np"]["price"] = float(update.message.text.strip().replace(" ",""))
        ...
        await update.message.reply_text("❌ Raqam kiriting!")
        return ADD_PROD_PRICE
    await update.message.reply_text("Tan narxini kiriting (so'm) — foyda hisoblash uchun (0 bo'lsa ham yozing):")
...
async def add_prod_cost(update, context):
    ...
    supabase.table("products").insert({"type":p["type"],"name":p["name"],"qty":p["qty"],"price":p["price"],"cost":cost,"sold":0}).execute()
    await update.message.reply_text(f"✅ *Qo'shildi:* {p['name']}\n📦 {p['qty']} | 💰 {fmt(p['price'])}", parse_mode="Markdown")
    return MAIN_MENU
...
async def edit_prod_field(update, context):
    ...
    if f=="delete":
        supabase.table("products").delete().eq("id",context.user_data["epid"]).execute()
        await update.callback_query.message.reply_text("🗑️ O'chirildi!")
        return MAIN_MENU
    context.user_data["epf"] = f
    lb = {"name":"yangi nomini","qty":"yangi miqdorini","price":"yangi narxini (so'm)","cost":"yangi tan narxini (so'm)"}
...
async def edit_prod_value(update, context):
    ...
    if f in ("qty","price","cost"):
        try: v = float(v)
            ...
            await update.message.reply_text("❌ Raqam kiriting!")
            return EDIT_PROD_VALUE
    supabase.table("products").update({f:v}).eq("id",context.user_data["epid"]).execute()
    await update.message.reply_text("✅ Yangilandi!")
    return MAIN_MENU

# ===== ADMIN: SAVDO =====
async def do_savdo(update, context):
    prods = supabase.table("products").select("*").eq("type","koylak").execute().data or []
    if not prods:
        await update.message.reply_text("❌ Koylak yo'q. Avval qo'shing.")
        return MAIN_MENU
...
async def sale_qty(update, context):
    ...
    await update.message.reply_text(f"💰 Narx (so'm): (standart: {fmt(context.user_data['ns']['pprice'])})")
...
async def sale_pay(update, context):
    ...
    ns = context.user_data["ns"]
    supabase.table("sales").insert({"product_name":ns["pname"],"prod_id":ns["pid"],"qty":ns["qty"],"price":ns["price"],"cost":ns["pcost"],"total":ns["total"],"profit":ns["profit"],"client":ns["client"],"pay_type":pay,"sale_date":today()}).execute()
    prod = supabase.table("products").select("qty,sold").eq("id",ns["pid"]).execute().data[0]
    supabase.table("products").update({"qty":prod["qty"]-ns["qty"],"sold":(prod.get("sold")or 0)+ns["qty"]}).eq("id",ns["pid"]).execute()
    if pay=="nasiya":
        ex = supabase.table("clients").select("*").eq("name",ns["client"]).execute().data
        if ex: supabase.table("clients").update({"debt":ex[0]["debt"]+ns["total"]}).eq("name",ns["client"]).execute()
        else: supabase.table("clients").insert({"name":ns["client"],"debt":ns["total"]}).execute()
...
async def exp_amount(update, context):
    ...
    supabase.table("expenses").insert({"type":ne["type"],"description":ne["desc"],"amount":amt,"expense_date":today()}).execute()
...
async def cb_rep(update, context):
    ...
    lb = {"daily":"Kunlik","weekly":"Haftalik (7 kun)","monthly":"Oylik"}[p]
    sales = supabase.table("sales").select("*").gte("sale_date",df).execute().data or []
    exps = supabase.table("expenses").select("*").gte("expense_date",df).execute().data or []
    clients = supabase.table("clients").select("*").execute().data or []
    prods = supabase.table("products").select("*").execute().data or []
    income = sum(s["total"] for s in sales if s.get("pay_type")!="nasiya")
    ...
    txt = f"📊 *{lb} hisobot — {today()}*\n\n"
    txt += f"💰 Daromad: *{fmt(income)}*\n"
    txt += f"📈 Sof foyda: *{fmt(profit)}*\n"
    txt += f"📋 Nasiya: *{fmt(nasiya)}*\n"
    txt += f"💸 Xarajat: *{fmt(expense)}*\n"
    txt += f"👔 Sotildi: *{qty} dona* ({len(sales)} ta sotuv)\n\n"
    txt += f"📦 *Ombor:*\n  💎 Jami: *{fmt(sv)}*\n"
    txt += f"  👔 {sum(p['qty'] for p in prods if p['type']=='koylak')} dona koylak\n"
    txt += f"  🧵 {sum(p['qty'] for p in prods if p['type']=='material')} metr material\n\n"
    if clients:
        txt += f"⚠️ *Qarzlar (jami {fmt(debt)}):*\n"
        for c in clients:
            if c["debt"]>0: txt += f"  • {c['name']}: {fmt(c['debt'])}\n"
    sm = {}
    for s in sales: sm[s["product_name"]] = sm.get(s["product_name"],0)+s["qty"]
    if sm:
        top = sorted(sm.items(),key=lambda x:x[1],reverse=True)[:3]
        txt += "\n🏆 *Top mahsulotlar:*\n"
        for i,(n,q) in enumerate(top): txt += f"  {i+1}. {n}: {q} dona\n"
...
async def do_ombor(update, context):
    ...
    for t,lb in [("koylak","👔 KOYLAKLAR"),("material","🧵 MATERIALLAR"),("detal","🔩 DETALLAR")]:
        ...
        if items:
            ...
            for p in items:
                w="⚠️ " if p["qty"]<5 else ""
                txt += f"  {w}*{p['name']}*\n    {p['qty']} dona × {fmt(p['price'])} = *{fmt(p['qty']*p['price'])}*\n"
            txt += "\n"
    txt += f"💎 *Jami ombor qiymati: {fmt(tv)}*"
    await update.message.reply_text(txt, parse_mode="Markdown")

# ===== ADMIN: ISHCHILAR =====
async def do_workers(update, context):
    ws = supabase.table("workers").select("*").execute().data or []
    if not ws:
    ...
    for w in ws:
        jobs = supabase.table("worker_jobs").select("*").eq("worker_id",w["id"]).eq("job_date",today()).execute().data or []
...
async def aw_login(update, context):
    ...
    if supabase.table("workers").select("id").eq("login",lg).execute().data:
...
async def aw_pass(update, context):
    ...
    supabase.table("workers").insert({"name":nw["name"],"role":nw["role"],"login":nw["login"],"pass":nw["pass"]}).execute()
...
async def ew_field(update, context):
    ...
    if f=="delete":
        supabase.table("workers").delete().eq("id",context.user_data["ewid"]).execute()
        await update.callback_query.message.reply_text("🗑️ O'chirildi!")
        return MAIN_MENU
...
async def ew_value(update, context):
    supabase.table("workers").update({context.user_data["ewf"]:update.message.text.strip()}).eq("id",context.user_data["ewid"]).execute()
    await update.message.reply_text("✅ Yangilandi!")
    return MAIN_MENU
...
async def assign_worker_cb(update, context):
    ...
    w = supabase.table("workers").select("*").eq("id",wid).execute().data[0]
    context.user_data["awname"] = w["name"]
    jobs = supabase.table("worker_jobs").select("*").eq("worker_id",wid).eq("job_date",today()).execute().data or []
    txt = f"📋 *{w['name']}* — bugun:\n"
    if jobs:
...
async def assign_qty(update, context):
    ...
    ex = supabase.table("worker_jobs").select("*").eq("worker_id",wid).eq("prod_id",prod["id"]).eq("job_date",today()).execute().data
    if ex: supabase.table("worker_jobs").update({"total":ex[0]["total"]+qty}).eq("id",ex[0]["id"]).execute()
    else: supabase.table("worker_jobs").insert({"worker_id":wid,"prod_id":prod["id"],"prod_name":prod["name"],"total":qty,"done":0,"job_date":today()}).execute()
    w = supabase.table("workers").select("*").eq("id",wid).execute().data[0]
...
async def w_vazifalar(update, context):
    ...
    jobs = supabase.table("worker_jobs").select("*").eq("worker_id",wid).eq("job_date",today()).execute().data or []
    if not jobs:
        await update.message.reply_text("📋 Bugun vazifa yo'q hali. Admin bersa xabar keladi! 🔔")
        return MAIN_MENU
    ...
    for j in jobs:
        ...
        ta+=j["total"]; da+=j["done"]
    tp = round(da/ta*100) if ta>0 else 0
    txt += f"📊 *Jami: {da}/{ta} ({tp}%)*"
    await update.message.reply_text(txt, parse_mode="Markdown")
    return MAIN_MENU
...
async def w_done_qty(update, context):
    ...
    supabase.table("worker_jobs").update({"done":new_done}).eq("id",job["id"]).execute()
    txt = f"✅ *{actual} ta saqlandi!*\n👔 {job['prod_name']}\n📊 {new_done}/{job['total']}"
    wid = context.user_data.get("worker_id")
    all_jobs = supabase.table("worker_jobs").select("*").eq("worker_id",wid).eq("job_date",today()).execute().data or []
...
async def w_tarix(update, context):
    wid = context.user_data.get("worker_id")
    jobs = supabase.table("worker_jobs").select("*").eq("worker_id",wid).order("job_date",desc=True).limit(20).execute().data or []
    if not jobs:
